...
void Sphere::render(Shader* sphereShader)
{
	modelMatrix = glm::translate(glm::mat4(1.0), pos);
	modelMatrix = Camera::getViewMatrix() * modelMatrix;
	glUniformMatrix4fv(glGetUniformLocation(sphereShader->handle(), "ModelViewMatrix"), 1, GL_FALSE, &modelMatrix[0][0]);


	normalMatrix = glm::inverseTranspose(glm::mat3(modelMatrix));
	glUniformMatrix3fv(glGetUniformLocation(sphereShader->handle(), "NormalMatrix"), 1, GL_FALSE, glm::value_ptr(normalMatrix));
...
void Sphere::createSphere(Shader* sphereShader, int level)
{
	if (!sphereShader->load("BasicView", "glslfiles/basicSphere.vert", "glslfiles/basicSphere.frag"))
	{
		cout << "failed to load shader" << endl;
	}
	else {
		cout << "SPHERE SHADER LOADED" << endl;;
	}
	...
	GLint vertexLocation = glGetAttribLocation(sphereShader->handle(), "in_Position");
	...
	GLint normalLocation = glGetAttribLocation(sphereShader->handle(), "in_Normal");
